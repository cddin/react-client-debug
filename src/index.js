...
import ConsoleLogUI from "./components/ConsoleLogUI";
...
function DebugHelper(props) {
  ...
  return (
    <div id="debugHelper" onClick={onDeveloperModeHandler}>
      {props.children}
...
export function updateNetworkList(val) {
  val.uniqueId = Math.floor(Math.random() * Math.floor(Math.random() * Date.now()));
